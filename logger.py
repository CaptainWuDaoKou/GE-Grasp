...
class Logger():
    ...
    def save_heightmaps(self, iteration, depth_heightmap, target_mask_heightmap, grasp_mask_heightmap, mode='grasp'):
        # saving data
        ori_heightmap = depth_heightmap.copy()
        ...
        # cv2.imwrite(os.path.join(self.target_mask_heightmaps_directory, '%06d.target.png' % iteration), (target_mask_heightmap * 255).astype(np.uint8))
        np.save(os.path.join(self.depth_heightmaps_directory, '%06d.depth.npy' % iteration), ori_heightmap)
        np.save(os.path.join(self.target_mask_heightmaps_directory, '%06d.target.npy' % iteration), target_mask_heightmap)
        if mode == 'grasp':
            # cv2.imwrite(os.path.join(self.grasp_mask_heightmaps_directory, '%06d.grasp.png' % iteration), (grasp_mask_heightmap * 255).astype(np.uint8)) 
            np.save(os.path.join(self.grasp_mask_heightmaps_directory, '%06d.grasp.npy' % iteration), grasp_mask_heightmap)
        else:
            # cv2.imwrite(os.path.join(self.push_mask_heightmaps_directory, '%06d.push.png' % iteration), (grasp_mask_heightmap * 255).astype(np.uint8)) 
            np.save(os.path.join(self.push_mask_heightmaps_directory, '%06d.push.npy' % iteration), grasp_mask_heightmap)
