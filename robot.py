...
class Robot(object):
    ...
    def sim_read_config_file(self, config_file):
        ...
        self.obj_labels = []
        # print('test_preset_cases', self.test_preset_cases)
    ...
    def close_gripper(self):
        ...
        sim_ret, RG2_gripper_handle = vrep.simxGetObjectHandle(self.sim_client, 'RG2_openCloseJoint',
                                                               vrep.simx_opmode_blocking)
        sim_ret, self.RG2_gripper_handle = vrep.simxGetObjectHandle(self.sim_client, 'RG2_openCloseJoint',
                                                               vrep.simx_opmode_blocking)
    ...
    def grasp(self, position, heightmap_rotation_angle, workspace_limits):
        ...
        if grasp_success:
            ...
            vrep.simxSetObjectPosition(self.sim_client, grasped_object_handle, -1,
                                       ...
                                       vrep.simx_opmode_blocking)
        # print('grasped something?', grasp_success)
        return grasp_success, grasped_object_name

    def retrieval(self, target_position, workspace_limits):

        x = target_position[0]
        y = target_position[1]
        z = target_position[2]
        x_min = workspace_limits[0][0]
        x_max = workspace_limits[0][1]
        y_min = workspace_limits[1][0]
        y_max = workspace_limits[1][1]
        dists = np.array([x-x_min, x_max-x, y-y_min, y_max-y])

        nearest = np.argmin(dists)
        # print('nearest: ', nearest)
        if nearest == 0:
            start_position = np.array([x_min-0.1, y, z-0.02])
            end_position = np.array([x+0.1, y, z-0.02])
        elif nearest == 1:
            start_position = np.array([x_max+0.1, y, z-0.02])
            end_position = np.array([x-0.1, y, z-0.02])
        elif nearest == 2:
            start_position = np.array([x, y_min-0.1, z-0.02])  
            end_position = np.array([x, y+0.1, z-0.02]) 
        else:
            start_position = np.array([x, y_max+0.1, z-0.02])
            end_position = np.array([x, y-0.1, z-0.02])

        push_length = np.abs(start_position - end_position) 

        location_above_pushing_point = start_position.copy()
        location_above_pushing_point[-1] += 0.1
        ...
        # Approach pushing start point
        self.move_to(start_position, None)

        # target_x = end_position[0]
        # target_y = end_position[1]

        # Move in pushing direction towards target location
        self.move_to([end_position[0], end_position[1], start_position[2]], None)

        # Move gripper to location above target
        self.move_to([end_position[0], end_position[1], location_above_pushing_point[2]], None)


    def push(self, start_position, random_rot_ind, target_position, workspace_limits):  # generate push data
        print('Executing: push at (%f, %f, %f)' % (start_position[0], start_position[1], start_position[2]))
        start_position[2] = start_position[2] + 0.04

        # Move gripper to location above pushing point
        pushing_point_margin = 0.1
        location_above_pushing_point = (start_position[0], start_position[1], start_position[2] + pushing_point_margin)
        ...
        random_rot_angle = (random_rot_ind - 1) * np.pi / 8
        random_rot_mat = np.array([[np.cos(random_rot_angle), -np.sin(random_rot_angle)],
                                   [np.sin(random_rot_angle), np.cos(random_rot_angle)]])
        push_direction = [target_position[0]-start_position[0], target_position[1]-start_position[1]]
        push_direction = push_direction / np.linalg.norm(push_direction)
        push_direction = np.dot(push_direction, random_rot_mat)

        push_end_position = start_position.copy()
        push_end_position[0: 2] += push_length * push_direction

        target_x = min(max(start_position[0] + push_direction[0] * push_length, workspace_limits[0][0]),
                       workspace_limits[0][1])
        target_y = min(max(start_position[1] + push_direction[1] * push_length, workspace_limits[1][0]),
                       workspace_limits[1][1])
        # Move in pushing direction towards target location
        self.move_to([target_x, target_y, start_position[2]], None)
        # Move gripper to location above grasp target
